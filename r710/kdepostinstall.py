#!/usr/bin/env python3
"""kdepostinstall.py - one-time setup for Fedora KDE Plasma on a Dell PowerEdge R710.

Run it as your normal (wheel/administrator) user, never as root:

    python3 kdepostinstall.py            # do everything
    python3 kdepostinstall.py --dry-run  # only print what would change
...
def step8_remote_session(major):
    ...
    if not session:
        ...
        write_root_file(SDDM_AUTOLOGIN, f"# Managed by {SCRIPT}.\n"
                                        f"[Autologin]\nUser={user}\nSession={session}\n")
